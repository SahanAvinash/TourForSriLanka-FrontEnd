  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-r from-primary-1 to-primary-2 flex items-center justify-center px-4 sm:px-6 py-10">
      <div className="w-full max-w-[1200px] flex flex-col md:flex-row items-center justify-center md:justify-between gap-10 md:gap-14 lg:gap-20">
        ...
        <div className="login-card-anim w-full max-w-[420px] flex flex-col items-center">
          ...
          <div className="w-full grid grid-cols-2 gap-x-4 xs:gap-x-6 sm:gap-x-[50px] gap-y-4 sm:gap-y-[10px] mt-[20px]">
            {ROLES.map(({ role, path, icon: Icon, title, desc }) => (
              <div
                ...
                className="w-full min-w-0 py-4 sm:py-0 sm:aspect-[150/170] cursor-pointer bg-border/30 rounded-[20px] drop-shadow-black flex flex-col items-center hover:scale-105 transition-all duration-300 px-2 hover:border border-primary-green/50"
              >
                <Icon className="w-[30px] h-[30px] sm:w-[40px] sm:h-[40px] text-primary-green mt-0 sm:mt-[30px] shrink-0" />
                <h1 className="w-full text-[13px] sm:text-[15px] font-bold text-text mt-[8px] sm:mt-[10px] text-center">
                  {title}
                </h1>
                <span className="w-full text-text text-[11px] sm:text-[12px] text-center p-[5px]">
                  {desc}
                </span>
              </div>
            ))}
